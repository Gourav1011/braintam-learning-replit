...
function TeacherSidebar({
  ...
  if (collapsed) {
    return (
      <div className="flex flex-col items-center py-3 border-r border-slate-800 bg-[#111827] flex-shrink-0" style={{ width: 40 }}>
        <button onClick={onCollapse} className="text-slate-500 hover:text-gray-300 mb-3">
  ...
  return (
    <div className="flex flex-col border-r border-slate-800 bg-[#111827] flex-shrink-0" style={{ width: 200 }}>
      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-800">
        <span className="text-[11px] font-bold text-gray-300 uppercase tracking-wide">
          Present ({liveStudents.length})
        </span>
        <button onClick={onCollapse} className="text-slate-500 hover:text-gray-300">
      ...
      <div className="px-2 py-1.5 border-b border-slate-800">
        <input
          ...
          className="w-full bg-slate-800/90 text-slate-100 text-[10px] rounded-lg px-2.5 py-1 border border-slate-700/60 outline-none placeholder-slate-500 focus:border-blue-500/80 transition-all"
        />
      </div>
      <div className="flex-1 overflow-y-auto">
        ...
        {filtered.map(s => {
          ...
          return (
            <div key={s.userId} className={`px-3 py-2 border-b border-slate-800/50 ${isSuggested ? "bg-purple-900/20" : "hover:bg-slate-800/40"}`}>
      ...
      {suggestedStudents.size > 0 && (
        <div className="px-3 py-1.5 border-t border-slate-800 flex-shrink-0">
  ...
  return (
    <div className="flex flex-col border-r border-slate-800 bg-[#111827] flex-shrink-0" style={{ width: 200 }}>
      ...
      <div className="grid grid-cols-3 border-b border-slate-800 flex-shrink-0">
        <div className="flex flex-col items-center py-2 border-r border-slate-800">
      ...
      {absentList.length > 0 && (
        <div className="border-b border-slate-800 flex-shrink-0 max-h-36 overflow-y-auto">
          <p className="text-[9px] text-red-400 font-bold px-3 pt-1.5 pb-0.5 uppercase tracking-wide">📞 Call Absent</p>
          {absentList.map(s => (
            <div key={s.userId} className="flex items-center justify-between px-3 py-1 hover:bg-slate-800/40">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {presentList.map(s => (
          <div key={s.userId} className="flex items-center gap-1.5 px-3 py-1.5 border-b border-slate-800/50 hover:bg-slate-800/40">
            <div className="w-1.5 h-1.5 rounded-full flex-shrink-0"
...
export default function LiveClassroom() {
  ...
  if (authLoading) {
    return (
      <div className="h-screen flex items-center justify-center bg-[#0B1020] text-white" style={{ fontFamily: "Poppins, sans-serif" }}>
        <p className="text-slate-400 text-sm">Checking class access…</p>
      </div>
    );
  }
  ...
  if (!authIdentity) {
    return (
      <div className="h-screen flex flex-col items-center justify-center gap-4 bg-[#0B1020] text-white px-6 text-center" style={{ fontFamily: "Poppins, sans-serif" }}>
        <h2 className="text-2xl font-bold">Sign in required</h2>
        <p className="text-slate-400 text-sm max-w-md">
          Please sign in with your authorised Braintam account to access this live class.
        </p>
        <a href="/sign-in" className="px-6 py-2 rounded-2xl font-bold text-white text-sm" style={{ background: "#FF6B1A" }}>
          Sign In
        </a>
      </div>
    );
  ...
  if (classEnded) {
    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center gap-6 bg-[#0B1020] text-white" style={{ fontFamily: "Poppins, sans-serif" }}>
        <div className="text-6xl">🔚</div>
        <h2 className="text-2xl font-bold">This class has ended</h2>
        <p className="text-slate-400 text-sm">The session is permanently closed and cannot be re-entered. Redirecting you…</p>
        <a href={redirectTarget} className="mt-2 px-6 py-2 rounded-2xl font-bold text-white text-sm" style={{ background: "#FF6B1A" }}>
  ...
  return (
    <>
      ...
      <div className={`live-classroom-rotate-prompt classroom-role-${role} bg-slate-950/95 backdrop-blur-md`}>
        <span style={{ fontSize: 40 }}>📱↻</span>
        <p className="font-bold text-lg">Rotate your phone</p>
        <p className="text-sm text-slate-400">Turn your device to landscape mode to join the live class</p>
      </div>
      <div className={`live-classroom-root classroom-role-${role} h-screen flex flex-col overflow-hidden bg-[#0B1020]${isLandscapeMobile ? " is-landscape-mobile" : ""}`} style={{ fontFamily: "Poppins, sans-serif" }}>
      ...
      {/* ── Main layout ── 
      <div className="classroom-main flex flex-1 overflow-hidden bg-slate-950 border-t border-slate-800/40 select-none">
        ...
        ═══════════════════════════════════════════════════ */}
        <div className="classroom-content flex flex-col relative bg-[#0B1020] flex-1 min-w-0">

          {/* Upload bar (teacher only, no presentation yet) */}
          {isStaff && !presentationUrl && !demoMode && (
            <div className="flex items-center gap-3 p-3 bg-[#111827] border-b border-slate-800 flex-shrink-0">
              <input
                ref={fileInputRef}
                type="file"
                accept=".pdf"
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-white rounded-2xl disabled:opacity-50 transition-all hover:opacity-90"
              ...
              <span className="text-xs text-slate-500">PDF only · max 200 MB · PPT/PPTX: export as PDF first</span>
              {presentationUrl && (
                <button
                  onClick={() => { setPresentationUrl(""); setUploadedFilename(null); socket?.emit("presentation:stop"); }}
                  className="ml-auto text-xs px-3 py-1.5 rounded-lg bg-gray-800 hover:bg-slate-700 text-slate-400"
                >✕ Clear</button>
              )}
            </div>
          )}
          ...
          <div ref={presentationPanelRef} className="relative flex-1 overflow-hidden bg-slate-950">
            ...
            <img
              src={BRAND_LOGO}
              alt="Braintam"
              className="absolute top-3 right-3 z-10 pointer-events-none drop-shadow-lg opacity-85 hover:opacity-100 transition-opacity"
              style={{ height: 44, opacity: 0.82 }}
            />

            {/* Class Paused banner — non-staff only */}
            {classPausedActive && !isStaff && (
              <div className="absolute inset-0 z-50 bg-slate-950/85 flex flex-col items-center justify-center gap-3 backdrop-blur-md transition-all">
                <div className="flex items-center gap-3 bg-[#111827] border border-yellow-600 text-yellow-300 text-base font-bold px-6 py-4 rounded-2xl shadow-2xl">
                  <Pause className="w-5 h-5" />
                  Class is temporarily paused
                </div>
                <p className="text-slate-400 text-sm">The teacher will be back shortly…</p>
              </div>
            )}
            ...
            {myOnStage && !isStaff && (
              <div className="absolute bottom-4 left-4 z-50 w-32 rounded-2xl overflow-hidden shadow-2xl border-2 border-green-500/70" style={{ background: "#0f172a" }}>
              ...
              <div
                ...
                {stageSlots
                  ...
                  <div
                    ...
                    className="rounded-2xl overflow-hidden shadow-2xl pointer-events-auto flex flex-col border-2 relative flex-shrink-0"
                    style={{
                      width: 120,
                      background: "#0f172a",
                    ...
                    <div className="relative flex items-center justify-center bg-[#111827]" style={{ aspectRatio: "4/3" }}>
                      <video
                    ...
                    {/* Name + mute row */}
                    <div className="px-2 py-1 bg-slate-950/90 flex items-center justify-between gap-1 border-t border-slate-800/80">
                    ...
                    {/* Teacher controls */}
                    {isStaff && (
                      <div className="flex justify-center gap-1 bg-[#111827]/95 px-1.5 py-1">
                        <button
          ...
          {!isStaff && !isMentor && raiseHandEnabled && (
            <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2">
              <button
                onClick={toggleHand}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-sm shadow-2xl backdrop-blur-md transition-all active:scale-95 ${myHandRaised ? "text-white border-2 border-amber-400 shadow-[0_0_12px_rgba(251,191,36,0.4)]" : "text-white hover:brightness-110"}`}
          ...
          {isStaff && (
            <div className="flex items-center gap-1.5 px-3 py-2 bg-[#111827] border-t border-slate-800 flex-shrink-0 flex-wrap">
              {/* Mode buttons */}
              <button onClick={() => setAnnotMode("none")}
                className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all text-xs font-bold ${annotMode === "none" ? "bg-gray-600 text-white" : "bg-gray-800 text-slate-500 hover:bg-slate-700"}`}
                title="Off">✕</button>
              <button onClick={() => setAnnotMode("pen")}
                className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${annotMode === "pen" ? "bg-orange-600 text-white" : "bg-gray-800 text-slate-400 hover:bg-slate-700"}`}
                title="Pen"><Pencil className="w-3.5 h-3.5" /></button>
              <button onClick={() => setAnnotMode("highlighter")}
                className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${annotMode === "highlighter" ? "bg-yellow-500 text-black" : "bg-gray-800 text-slate-400 hover:bg-slate-700"}`}
                title="Highlight"><Highlighter className="w-3.5 h-3.5" /></button>
              <button onClick={() => setAnnotMode("eraser")}
                className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${annotMode === "eraser" ? "bg-red-700 text-white" : "bg-gray-800 text-slate-400 hover:bg-slate-700"}`}
              ...
              {(annotMode === "pen" || annotMode === "eraser") && (
                <div className="flex items-center gap-1.5 border-l border-gray-700 pl-2">
                  <span className="text-[9px] text-slate-500 uppercase tracking-wide">Size</span>
                  ...
                  <span className="text-[10px] text-slate-400 w-4 text-right">{penWidth}</span>
                </div>
              )}
              ...
              <div className="flex items-center gap-1 border-l border-gray-700 pl-2">
                <button onClick={handleUndo} title="Undo (Ctrl+Z)"
                  className="w-8 h-8 rounded-lg flex items-center justify-center bg-gray-800 text-slate-400 hover:bg-slate-700 transition-all active:scale-95">
              ...
              <div className="flex items-center gap-1 border-l border-gray-700 pl-2">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 h-8 rounded-lg font-semibold text-[11px] transition-all ${raiseHandEnabled ? "bg-yellow-600/30 text-yellow-300 border border-yellow-700/50" : "bg-gray-800 text-slate-400 hover:bg-slate-700"}`}
              ...
              {presentationUrl && (
                <div className="flex items-center gap-1 ml-auto border-l border-gray-700 pl-2">
                  ...
                  <span className="text-[10px] text-slate-400 px-1">{currentSlide}{totalSlides > 0 ? ` / ${totalSlides}` : ""}</span>
                  <button onClick={() => navigateSlide("next")}
                    className="w-8 h-8 rounded-lg flex items-center justify-center bg-gray-800 hover:bg-gray-600 text-white transition-all active:scale-95"
                    title="Next"><ChevronRight className="w-4 h-4" /></button>
                  <button
                    onClick={() => { setPresentationUrl(""); setUploadedFilename(null); socket?.emit("presentation:stop"); }}
                    className="w-8 h-8 rounded-lg flex items-center justify-center bg-gray-800 hover:bg-red-900 text-slate-400 hover:text-red-400 transition-all ml-1"
        ...
        ═══════════════════════════════════════════════════ */}
        <div className="classroom-sidebar flex flex-col border-l border-slate-800 bg-[#111827] flex-shrink-0" style={{ width: "20%", minWidth: 190, maxWidth: 280 }}>
          ...
          <div
            className="classroom-teacher-video relative bg-slate-950 flex-shrink-0 overflow-hidden border-b border-slate-800/80 shadow-md group"
            style={{ width: "100%", aspectRatio: "16 / 9" }}
          >
            {/* Label */}
            <div className="absolute top-2 left-2 z-10 flex items-center gap-1.5">
              <span className="text-[9px] text-slate-400 font-semibold uppercase tracking-wide">Teacher</span>
            </div>
            ...
            <img
              src={ROUND_LOGO}
              alt="Braintam"
              className="absolute top-2 right-2 z-10 rounded-full pointer-events-none drop-shadow-md border border-slate-700/50"
            ...
            {isStaff ? (
              ...
              teacherInfo.online ? (
                ...
                <>
                  ...
                  {!livekit.teacherPresent && !isShowingDemo && (
                    <div className="flex flex-col items-center justify-center h-full gap-2">
                      ...
                      <p className="text-[9px] text-slate-500">Camera off</p>
                    </div>
                  )}
                </>
              ) : (
                ...
                <div className="flex flex-col items-center justify-center h-full gap-2 px-3 text-center">
                  ...
                  <p className="text-[8px] text-slate-500 leading-tight">Stream will restore automatically</p>
                </div>
              )
            ) : (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-2 px-3 text-center">
                ...
                <p className="text-[10px] text-slate-500 leading-tight">Waiting for teacher to join…</p>
              </div>
            )}
          </div>
          ...
          {!isStaff && (
            <div className="classroom-mobile-info flex items-center justify-between px-3 flex-shrink-0 gap-1 bg-slate-950/90 border-b border-orange-500/40 backdrop-blur-sm" style={{ height: 32 }}>
              {/* Left: subject | LIVE */}
              <div className="flex items-center gap-1.5 min-w-0 flex-1 overflow-hidden">
                <span className="text-[9px] text-slate-400 truncate max-w-[110px] flex-shrink-0">{title}</span>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <span className={`text-[9px] font-bold uppercase tracking-wide ${connected ? "text-red-400" : "text-slate-500"}`}>
                    {connected ? "LIVE" : "…"}
                  </span>
                </div>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button
                  ...
                  className="w-7 h-7 rounded-lg bg-gray-800 text-slate-400 flex items-center justify-center active:scale-90 transition-all"
                >
          ...
          {(!isStaff && !isMentor) ? (
            <div className="flex flex-col flex-1 overflow-hidden">
                  ...
                  {activePoll.options.map(opt => {
                    ...
                    return (
                      <button key={opt.id} onClick={() => submitPoll(opt.id)} disabled={!!myPollAnswer}
                        className={`w-full text-left rounded-2xl px-3 py-2 text-xs font-semibold transition-all ${chosen ? "text-white" : "text-gray-200 bg-gray-800 hover:bg-slate-700 disabled:hover:bg-slate-800"}`}
                ...
                {chat
                  ...
                  .map(msg => (
                    <div
                      ...
                      className={
                        msg.role === "teacher"
                          ? "bg-red-950/50 border border-red-700/60 rounded-2xl px-2.5 py-2"
                          : msg.isAnnouncement
                    ...
                    >
                      <div className="flex items-center gap-1.5 mb-0.5">
                        <span className="text-[10px] font-bold text-slate-400">{msg.name}</span>
              ...
              {raiseHandEnabled && (
                <div className="px-2 pt-1 flex-shrink-0">
                  <button onClick={toggleHand}
                    className={`w-full py-1.5 text-xs font-bold rounded-2xl transition-all ${myHandRaised ? "bg-yellow-500 text-white" : "bg-gray-800 text-gray-300 hover:bg-slate-700"}`}>
              ...
              {chatBlocked ? (
                ...
                <div className="mx-2 mb-2 mt-1 px-3 py-2.5 rounded-lg bg-gray-800/80 border border-gray-700 text-slate-400 text-xs font-medium flex-shrink-0 text-center">
                  🔒 Chat is paused by your teacher. Keep learning — chat will reopen soon.
                </div>
              ) : (
                <>
                <div className="relative px-2 pt-2 border-t border-slate-800 flex-shrink-0">
                  {showQuickChat && (
                    <div className="absolute left-2 bottom-full mb-2 z-50 w-[260px] max-w-[calc(100vw-24px)] max-h-[300px] overflow-y-auto rounded-2xl border border-gray-700 bg-[#111827] shadow-2xl p-1.5">
                      <div className="flex items-center justify-between px-2 pb-1">
                        ...
                        <button
                          ...
                          className="w-6 h-6 rounded-full text-slate-400 hover:bg-slate-800 hover:text-white"
                        >
                          ×
                        </button>
                      </div>
                      ...
                      <div className="grid gap-1">
                        {quickChatMessages.map((text) => (
                          <button
                            ...
                            className="w-full text-left px-2.5 py-1.5 rounded-lg text-[11px] leading-4 font-semibold text-gray-100 hover:bg-slate-800 active:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed"
                          >
                            {text}
                          </button>
                        ))}
                  ...
                  <button
                    ...
                    disabled={!isStaff && !isMentor && (chatBlocked || isChatMuted)}
                    className="w-10 h-10 rounded-full bg-gray-800 border border-gray-700 flex items-center justify-center text-lg hover:bg-slate-700 active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                ...
                <div className="classroom-composer border-t border-slate-800 flex gap-2 flex-shrink-0 p-2">
                  <input
                    ref={chatInputRef}
                    className="flex-1 min-w-0 bg-slate-800/90 text-slate-100 rounded-xl px-3 py-1.5 border border-slate-700/60 outline-none placeholder-slate-500 focus:border-blue-500/80 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                    style={{ fontSize: 13 }}
                  ...
                  <button
                    ...
                    className="w-11 h-11 rounded-2xl text-white flex items-center justify-center flex-shrink-0 hover:opacity-90 active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
          ...
          <div className="flex border-b border-slate-800 flex-shrink-0">
            <button onClick={() => setPanelMode("chat")}
              className={`flex-1 flex items-center justify-center gap-1 py-2.5 text-[11px] font-semibold border-b-2 transition-all ${panelMode === "chat" ? "border-blue-500 text-blue-400" : "border-transparent text-slate-500 hover:text-gray-300"}`}>
              <MessageSquare className="w-3 h-3" /> Chat
            </button>
            <button onClick={() => setPanelMode("poll")}
              className={`flex-1 flex items-center justify-center gap-1 py-2.5 text-[11px] font-semibold border-b-2 transition-all ${panelMode === "poll" ? "border-orange-500 text-orange-400" : "border-transparent text-slate-500 hover:text-gray-300"}`}>
            ...
            {(isStaff || isMentor) && (
              <button onClick={() => setPanelMode("staffchat")}
                className={`flex-1 flex items-center justify-center gap-1 py-2.5 text-[11px] font-semibold border-b-2 transition-all ${panelMode === "staffchat" ? "border-purple-500 text-purple-400" : "border-transparent text-slate-500 hover:text-gray-300"}`}>
                🔒 Staff
              </button>
            )}
          </div>
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
              ...
              {isStaff && (
                <div className="px-3 py-2 border-b border-slate-800 flex-shrink-0 flex items-center gap-2">
                  <button
                    onClick={() => toggleRaiseHandFeature(!raiseHandEnabled)}
                    className={`text-[10px] px-2.5 py-1 rounded-full font-semibold transition-all ${raiseHandEnabled ? "bg-yellow-600/30 text-yellow-300 border border-yellow-700/30" : "bg-gray-800 text-slate-400 hover:bg-slate-700"}`}>
                    ✋ {raiseHandEnabled ? "Close Q&A" : "Open Q&A"}
                  </button>
                  <button
                    onClick={() => {
                    ...
                    className={`text-[10px] px-2.5 py-1 rounded-full font-semibold transition-all flex items-center gap-1 ${chatMuted ? "bg-red-700/30 text-red-300 border border-red-700/40" : "bg-gray-800 text-slate-400 hover:bg-slate-700"}`}>
              ...
              {!isStaff && chatBlocked ? (
                ...
                <div className="mx-2 mb-2 px-3 py-2.5 rounded-lg bg-gray-800/80 border border-gray-700 text-slate-400 text-xs font-medium flex-shrink-0 text-center">
                  🔒 Chat is paused by your teacher. Keep learning — chat will reopen soon.
                </div>
              ) : (
                <>
              ...
              {!isStaff && !isMentor && raiseHandEnabled && (
                <div className="p-2 border-t border-slate-800 flex-shrink-0">
                  <button onClick={toggleHand}
                    className={`w-full py-2 text-xs font-bold rounded-2xl transition-all ${myHandRaised ? "bg-yellow-500 text-white" : "bg-gray-800 text-gray-300 hover:bg-slate-700"}`}>
                    <Hand className="w-3 h-3 inline mr-1" />{myHandRaised ? "Lower Hand" : "Raise Hand"}
                  </button>
                </div>
              )}
          ...
          {panelMode === "poll" && (
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
              {isStaff && !activePoll && (
                !showPollForm ? (
                  <button onClick={() => setShowPollForm(true)} className="w-full py-2.5 text-sm font-bold text-white rounded-2xl" style={{ background: ORANGE }}>
                    + Create Poll
                  </button>
                ) : (
                  <div className="space-y-2">
                    <input className="w-full bg-gray-800 text-white text-xs rounded-2xl px-3 py-2 border border-gray-700 outline-none placeholder-gray-600"
                      placeholder="Question…" value={pollQ} onChange={e => setPollQ(e.target.value)} />
                    {/* Sprint 1 — options with correct-answer radio */}
                    <p className="text-[9px] text-slate-500 font-semibold uppercase tracking-wide">Options · tap ✓ to mark correct</p>
                    {pollOpts.map((opt, i) => {
                      ...
                      return (
                        <div key={i} className="flex gap-1.5 items-center">
                          <button
                            ...
                            className={`w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center text-[9px] font-black border transition-all ${isCorrect ? "bg-green-500 border-green-400 text-white" : "border-gray-600 text-slate-500 hover:border-green-600"}`}
                            title="Mark as correct answer"
                          >✓</button>
                          <input
                            className={`flex-1 bg-gray-800 text-white text-xs rounded-2xl px-3 py-2 border outline-none placeholder-gray-600 ${isCorrect ? "border-green-600" : "border-gray-700"}`}
                    ...
                    <div className="flex gap-2">
                      <button onClick={launchPoll} className="flex-1 py-2 text-xs font-bold text-white rounded-2xl" style={{ background: ORANGE }}>🚀 Launch</button>
                      <button onClick={() => { setShowPollForm(false); setPollCorrectIdx(null); }} className="px-3 py-2 text-xs text-slate-400 bg-gray-800 rounded-2xl hover:bg-slate-700">Cancel</button>
                    </div>
                  </div>
                )
              )}
                ...
                <div className="space-y-2">
                  ...
                  {activePoll.options.map(opt => {
                    ...
                    return (
                      <button key={opt.id} onClick={() => submitPoll(opt.id)} disabled={!!myPollAnswer && !isStaff}
                        className={`w-full text-left rounded-2xl px-3 py-2.5 text-xs font-semibold relative overflow-hidden transition-all ${chosen ? "text-white" : "text-gray-200 bg-gray-800 hover:bg-slate-700 disabled:hover:bg-slate-800"}`}
                        style={chosen ? { background: NAVY } : {}}>
                        {isStaff && pollTotal > 0 && <div className="absolute inset-y-0 left-0 bg-blue-500/20 rounded-2xl" style={{ width: `${pct}%` }} />}
                        <span className="relative flex justify-between">
                          <span>{opt.id}. {opt.text}</span>
                          {isStaff && pollTotal > 0 && <span className="text-slate-400 ml-2">{pct}%</span>}
                        </span>
                      </button>
                    );
                  })}
                  {isStaff && (
                    <button onClick={() => socket?.emit("showLeaderboard")}
                      className="w-full py-2 text-xs font-bold text-white rounded-2xl flex items-center justify-center gap-1.5" style={{ background: "#7C3AED" }}>
          ...
          {panelMode === "staffchat" && (isStaff || isMentor) && (
            <div className="flex flex-col flex-1 overflow-hidden">
              ...
              <div className="flex gap-1.5 px-2 py-2 border-t border-slate-800 flex-shrink-0">
                <input
      ...
      {leaderboard && leaderboard.length > 0 && (
        <div className="fixed inset-0 flex items-center justify-center bg-slate-950/80 backdrop-blur-md z-50 transition-all">
          <div className="rounded-3xl p-8 text-center shadow-2xl" style={{ background: NAVY, minWidth: 320, maxHeight: "80vh" }}>
            ...
            <div className="space-y-1.5 overflow-y-auto pr-1" style={{ maxHeight: "50vh" }}>
              {leaderboard.map(e => (
                <div key={e.rank} className="flex items-center gap-3 bg-white/10 rounded-2xl px-4 py-1.5">
      ...
      {micInvite && (
        <div className="fixed inset-0 flex items-end justify-center pb-24 z-50 pointer-events-none">
          <div className="pointer-events-auto animate-bounce-once rounded-2xl shadow-2xl border border-emerald-500/50 px-5 py-4 flex flex-col items-center gap-3 max-w-xs w-full mx-4 backdrop-blur-md bg-slate-900/95"
            ...
            <div className="flex gap-3 w-full">
              <button
                ...
                className="flex-1 py-2 text-sm font-bold text-white rounded-2xl transition-all hover:opacity-90"
              ...
              <button
                ...
                className="flex-1 py-2 text-sm font-bold text-gray-300 rounded-2xl bg-gray-700 hover:bg-gray-600 transition-all"
              >
                Decline
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 transition-all">
          <div className="relative rounded-3xl overflow-hidden shadow-2xl w-full max-w-sm"
            ...
            <div className="mx-6 rounded-2xl p-4 mb-4 text-center"
              ...
              <p className="text-slate-400 text-[11px] mb-1">Original Price</p>
              <p className="text-slate-500 text-sm line-through">₹{Number(origPrice).toLocaleString("en-IN")}</p>
            ...
            <div className="px-6 pb-6">
              ...
              <p className="text-slate-500 text-[10px] text-center mt-2">No hidden charges · Secure payment</p>
            </div>
      ...
      {showDiagnostics && (
        <div className="fixed bottom-2 left-2 z-[9999] w-72 rounded-lg bg-black/90 border border-gray-700 text-[9px] font-mono text-gray-300 p-2 space-y-0.5 pointer-events-none select-none">
          ...
          <p className="text-slate-500 font-semibold pt-0.5">─ Identity</p>
          ...
          <p className="text-slate-500 font-semibold pt-0.5">─ Socket.IO</p>
          ...
          <p>Room match: <span className={classroomJoined ? (classroomJoined.roomName === `session-${sessionId}` ? "text-green-400" : "text-red-400") : "text-slate-500"}>{classroomJoined ? (classroomJoined.roomName === `session-${sessionId}` ? "✓ yes" : "✗ MISMATCH") : "—"}</span></p>
          ...
          <p className="text-slate-500 font-semibold pt-0.5">─ LiveKit</p>
          ...
          {isStaff ? (
            ...
            <>
              <p>Local Camera: <span className="text-slate-500">Not permitted</span></p>
              <p>Local Microphone: <span className="text-slate-500">Not permitted</span></p>
      ...
      {isStaff && showQnaOverlay && (
        <div
          className="fixed inset-0 z-[200] flex items-center justify-center backdrop-blur-md bg-slate-950/80 transition-all"
          style={{ background: "rgba(0,0,0,0.65)", backdropFilter: "blur(4px)" }}
          onClick={e => { if (e.target === e.currentTarget) setShowQnaOverlay(false); }}
        >
          <div className="bg-[#111827] border border-yellow-700/40 rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800">
              ...
              <button onClick={() => setShowQnaOverlay(false)} className="text-slate-500 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            {/* List */}
            <div className="max-h-80 overflow-y-auto divide-y divide-gray-800">
              {visibleHands.length === 0 ? (
                <p className="text-slate-500 text-sm text-center py-8">No students have raised their hand yet.</p>
              ) : visibleHands.map(h => {
                const alreadyOnStage = stageSlots.some(s => s.studentId === h.uid);
                const stageFull = stageSlots.length >= 5;
                const isSuggested = suggestedStudents.has(h.uid);
                return (
                  <div key={h.uid} className={`flex items-center justify-between px-5 py-3 ${isSuggested ? "bg-purple-900/20" : "hover:bg-slate-800/50"}`}>
                    <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {stageFull && !alreadyOnStage && <span className="text-xs text-slate-500">Stage full</span>}
                    </div>
                  </div>
                );
              })}
            ...
            <div className="px-5 py-3 border-t border-slate-800 flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs px-4 py-1.5 rounded-lg bg-gray-800 hover:bg-slate-700 text-gray-300 font-semibold"
              >Dismiss</button>
            </div>
          </div>
        </div>
