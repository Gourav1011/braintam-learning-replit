...
import { FileText, Calendar, Clock, CheckCircle, AlertCircle, Send } from "lucide-react";
          ...
          <div className="space-y-4">
            {(homework ?? []).map((hw, i) => {
              ...
              return (
                ...
                >
                  <Card className={`border-2 ${isUrgent && hw.status === "pending" ? "border-red-300 bg-red-50/30" : "border-border"}`}>
                    <CardContent className="p-5 flex items-start gap-5">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${hw.status === "graded" ? "bg-green-100" : hw.status === "submitted" ? "bg-blue-100" : isUrgent ? "bg-red-100" : "bg-yellow-100"}`}>
