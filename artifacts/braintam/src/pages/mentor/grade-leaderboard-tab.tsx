                    ...
                    return (
                      <div key={entry!.mentorId} className="flex flex-col items-center gap-2 flex-1">
                        ...
                        <div className="text-center">
                          ...
                          <div className="text-[9px] text-gray-400">{entry!.converted}/{entry!.assigned}</div>
                        </div>
            ...
            {(rest.length > 0 || lb.length <= 3) && (
              <div className="rounded-2xl overflow-hidden border border-gray-200 bg-white"
                ...
                <div className="grid items-center px-5 py-2.5 border-b border-gray-200"
                  style={{ gridTemplateColumns: "48px 1fr 64px 80px", background: "#F8FAFF" }}>
                  ...
                  <div className="text-[11px] font-bold text-gray-500 uppercase tracking-widest text-center">Leads</div>
                  <div className="text-[11px] font-bold text-gray-500 uppercase tracking-widest text-right">Conv %</div>
                </div>

                {(rest.length > 0 ? rest : lb).map((row, idx) => {
                  const isMe = row.mentorId === myId;
                  const displayRank = rest.length > 0 ? row.rank : idx + 1;
                  return (
                    <div key={row.mentorId} className="grid items-center px-5 py-3.5"
                      ...
                      <div className="font-bold text-sm" style={{ color: NAVY }}>#{displayRank}</div>
                      <div className="font-semibold text-sm text-gray-900 truncate">
                        {row.mentorName}{isMe ? " (You)" : ""}
                      </div>
                      <div className="text-center text-sm text-gray-500">{row.assigned}</div>
