...
import { useGetStudentProfile, useGetStudentProgress, useUpdateStudentProfile, getGetStudentProfileQueryKey, getGetStudentProgressQueryKey } from "@workspace/api-client-react";
...
import { User, Star, Trophy, BookOpen, CheckSquare, School, Phone, Mail, Edit } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleUpdate = () => {
    updateMutation.mutate({ data: { name: editName, school: editSchool } });
  };
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <Card className="border-2">
              <CardContent className="p-6 space-y-5">
                {profileLoading ? (
                  <div className="space-y-3"><Skeleton className="w-24 h-24 rounded-full mx-auto" /><Skeleton className="w-40 h-6 mx-auto" /></div>
                ) : (
                  <>
                    <div className="text-center">
                      <Avatar className="w-24 h-24 mx-auto mb-4 text-2xl font-bold border-4 border-primary/20">
                        <AvatarFallback className="bg-gradient-to-br from-secondary to-primary text-white text-2xl font-bold">
                          {initials}
                        </AvatarFallback>
                      </Avatar>
                      <h2 className="text-xl font-bold" data-testid="profile-name">{profile?.name}</h2>
        ...
        <Dialog open={editing} onOpenChange={setEditing}>
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleUpdate} disabled={updateMutation.isPending || !editName.trim()} data-testid="save-profile-btn">
