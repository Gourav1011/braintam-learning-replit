...
interface LiveClass { id: number; title: string; teacher: string; scheduledAt: string; status: string; grade: number; duration: number; joinUrl: string | null; }
...
export default function TeacherPage() {
  ...
  const [testForm, setTestForm] = useState({ title: "", subjectId: "", grade: "", courseId: "", scheduledAt: "", duration: "30" });
  const [questions, setQuestions] = useState<{ text: string; options: [string, string, string, string]; correctOption: number }[]>([
  ...
  async function createHomework() {
    setBusy(true);
    const r = await apiFetch("/teacher/homework", { method: "POST", body: JSON.stringify({ ...hwForm, subjectId: Number(hwForm.subjectId), grade: Number(hwForm.grade), courseId: hwForm.courseId ? Number(hwForm.courseId) : null, maxMarks: Number(hwForm.maxMarks) }) });
    if (r.ok) { flash("Homework posted!"); setShowHwForm(false); setHwForm({ title: "", subjectId: "", grade: "", courseId: "", dueDate: "", description: "", maxMarks: "10" }); loadAll(); }
  ...
  async function createLiveClass() {
    setBusy(true);
    const r = await apiFetch("/teacher/live-classes", { method: "POST", body: JSON.stringify({ ...lcForm, subjectId: Number(lcForm.subjectId), grade: Number(lcForm.grade), courseId: lcForm.courseId ? Number(lcForm.courseId) : null, duration: Number(lcForm.duration) }) });
    if (r.ok) { flash("Live class scheduled!"); setShowLcForm(false); setLcForm({ title: "", subjectId: "", grade: "", courseId: "", scheduledAt: "", duration: "60", joinUrl: "" }); loadAll(); }
    else { const d = await r.json(); flash(d.error ?? "Error", false); }
    setBusy(false);
  }
  ...
  async function createTest() {
    ...
    const r = await apiFetch("/teacher/tests", { method: "POST", body: JSON.stringify({ title: testForm.title, subjectId: Number(testForm.subjectId), grade: Number(testForm.grade), courseId: testForm.courseId ? Number(testForm.courseId) : null, scheduledAt: testForm.scheduledAt, duration: Number(testForm.duration), questions: validQs }) });
    if (r.ok) { flash("Test created!"); setShowTestForm(false); setTestForm({ title: "", subjectId: "", grade: "", courseId: "", scheduledAt: "", duration: "30" }); setQuestions([{ text: "", options: ["", "", "", ""], correctOption: 0 }]); loadAll(); }
    else { const d = await r.json(); flash(d.error ?? "Error", false); }
    setBusy(false);
  }
  ...
  async function deleteTest(id: number) {
    await apiFetch(`/teacher/tests/${id}`, { method: "DELETE" }); loadAll();
  }

  async function gradeSubmission() {
    if (!grading) return;
    setBusy(true);
    const path = grading.type === "homework" ? `/teacher/submissions/homework/${grading.id}/grade` : `/teacher/submissions/assignments/${grading.id}/grade`;
  ...
  async function saveAttendance() {
    ...
    const r = await apiFetch(`/teacher/live-classes/${attendanceClassId}/attendance`, { method: "POST", body: JSON.stringify({ records: attendanceRecords.map(r => ({ studentId: r.studentId, present: r.present })) }) });
  ...
  function addQuestion() {
    setQuestions(prev => [...prev, { text: "", options: ["", "", "", ""], correctOption: 0 }]);
  }
  ...
  function updateOption(qi: number, oi: number, value: string) {
    setQuestions(prev => prev.map((q, idx) => idx === qi ? { ...q, options: q.options.map((o, j) => j === oi ? value : o) as [string, string, string, string] } : q));
  }
        ...
        {TABS.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              {t.id === "submissions" && pendingCount > 0 && <span className="ml-1 bg-red-500 text-white rounded-full text-xs w-4 h-4 flex items-center justify-center leading-none">{pendingCount}</span>}
            </button>
          );
        })}
      </div>
      ...
      <div className="p-5 space-y-5">
        ...
        {tab === "dashboard" && dash && (
          <div className="space-y-5">
            ...
            <div className="grid md:grid-cols-3 gap-3">
              {[
                { label: "Post Homework", sub: "Assign to students", icon: FileText, action: () => { setShowHwForm(true); setTab("homework"); }, color: ORANGE },
        ...
        {tab === "courses" && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {courses.length === 0 && (
              <div className="col-span-3 py-16 text-center"><BookOpen className="w-10 h-10 text-gray-300 mx-auto mb-3" /><p className="text-gray-400 font-medium">No courses assigned</p><p className="text-xs text-gray-300 mt-1">Ask your admin to assign you to a course</p></div>
            )}
          </div>
        )}
        ...
        {tab === "homework" && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Button size="sm" onClick={() => setShowHwForm(!showHwForm)} className="text-white gap-1.5" style={{ background: ORANGE }}><Plus className="w-3.5 h-3.5" /> Post</Button>
            </div>
            {showHwForm && (
              <div className="bg-white rounded-2xl p-5 border border-orange-200 shadow-sm space-y-3">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  ...
                  <Select value={hwForm.subjectId} onValueChange={v => setHwForm(p => ({ ...p, subjectId: v }))}><SelectTrigger><SelectValue placeholder="Subject *" /></SelectTrigger><SelectContent>{subjects.map(s => <SelectItem key={s.id} value={String(s.id)}>{s.name}</SelectItem>)}</SelectContent></Select>
                  <Input placeholder="Grade *" type="number" min="1" max="10" value={hwForm.grade} onChange={e => setHwForm(p => ({ ...p, grade: e.target.value }))} />
                  <Select value={hwForm.courseId} onValueChange={v => setHwForm(p => ({ ...p, courseId: v }))}><SelectTrigger><SelectValue placeholder="Course (optional)" /></SelectTrigger><SelectContent><SelectItem value="">No course</SelectItem>{courses.map(c => <SelectItem key={c.id} value={String(c.id)}>{c.title}</SelectItem>)}</SelectContent></Select>
                  ...
                  <Textarea placeholder="Description" value={hwForm.description} onChange={e => setHwForm(p => ({ ...p, description: e.target.value }))} className="sm:col-span-2" rows={2} />
                </div>
                <div className="flex gap-2"><Button size="sm" onClick={createHomework} disabled={busy || !hwForm.title || !hwForm.subjectId || !hwForm.grade || !hwForm.dueDate} className="text-white" style={{ background: ORANGE }}>Post</Button><Button size="sm" variant="ghost" onClick={() => setShowHwForm(false)}>Cancel</Button></div>
              </div>
            )}
            <div className="space-y-3">
              {homework.map(h => (
                <div key={h.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center justify-between">
                  <div><div className="font-semibold text-sm" style={{ color: NAVY }}>{h.title}</div><div className="text-xs text-gray-400 mt-0.5">{h.subjectName} · Grade {h.grade} · Due {new Date(h.dueDate).toLocaleDateString("en-IN")}</div></div>
                  <span className="text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded-full font-semibold">{h.maxMarks} marks</span>
                </div>
              ))}
              {homework.length === 0 && !showHwForm && <div className="py-12 text-center"><FileText className="w-8 h-8 text-gray-300 mx-auto mb-2" /><p className="text-gray-400 text-sm">No homework posted yet</p></div>}
            </div>
          </div>
        )}
        ...
        {tab === "live" && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Button size="sm" onClick={() => setShowLcForm(!showLcForm)} className="text-white gap-1.5" style={{ background: ORANGE }}><Plus className="w-3.5 h-3.5" /> Schedule</Button>
            </div>
            {showLcForm && (
              <div className="bg-white rounded-2xl p-5 border border-orange-200 shadow-sm space-y-3">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  ...
                  <Select value={lcForm.subjectId} onValueChange={v => setLcForm(p => ({ ...p, subjectId: v }))}><SelectTrigger><SelectValue placeholder="Subject *" /></SelectTrigger><SelectContent>{subjects.map(s => <SelectItem key={s.id} value={String(s.id)}>{s.name}</SelectItem>)}</SelectContent></Select>
                  <Input placeholder="Grade *" type="number" min="1" max="10" value={lcForm.grade} onChange={e => setLcForm(p => ({ ...p, grade: e.target.value }))} />
                  <Select value={lcForm.courseId} onValueChange={v => setLcForm(p => ({ ...p, courseId: v }))}><SelectTrigger><SelectValue placeholder="Course (optional)" /></SelectTrigger><SelectContent><SelectItem value="">No course</SelectItem>{courses.map(c => <SelectItem key={c.id} value={String(c.id)}>{c.title}</SelectItem>)}</SelectContent></Select>
                ...
                <div className="flex gap-2"><Button size="sm" onClick={createLiveClass} disabled={busy || !lcForm.title || !lcForm.subjectId || !lcForm.grade || !lcForm.scheduledAt} className="text-white" style={{ background: ORANGE }}>Schedule</Button><Button size="sm" variant="ghost" onClick={() => setShowLcForm(false)}>Cancel</Button></div>
              </div>
            )}
            <div className="space-y-3">
              {liveClasses.map(lc => (
                <div key={lc.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {lc.status === "upcoming" && <button onClick={() => updateClassStatus(lc.id, "live")} className="text-xs px-2 py-1 rounded-lg bg-green-50 text-green-600 hover:bg-green-100 transition-colors flex items-center gap-1"><Play className="w-3 h-3" />Start</button>}
                      {lc.status === "live" && <button onClick={() => updateClassStatus(lc.id, "completed")} className="text-xs px-2 py-1 rounded-lg bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors flex items-center gap-1"><Square className="w-3 h-3" />End</button>}
                      {lc.joinUrl && <a href={lc.joinUrl} target="_blank" rel="noreferrer" className="text-xs px-2 py-1 rounded-lg bg-orange-50 text-orange-600 hover:bg-orange-100 transition-colors">Join</a>}
              ...
              {liveClasses.length === 0 && !showLcForm && <div className="py-12 text-center"><Video className="w-8 h-8 text-gray-300 mx-auto mb-2" /><p className="text-gray-400 text-sm">No live classes yet</p></div>}
            </div>
          </div>
        )}
        ...
        {tab === "tests" && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Button size="sm" onClick={() => setShowTestForm(!showTestForm)} className="text-white gap-1.5" style={{ background: ORANGE }}><Plus className="w-3.5 h-3.5" /> Create Test</Button>
            </div>
            ...
            {showTestForm && (
              <div className="bg-white rounded-2xl p-5 border border-orange-200 shadow-sm space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  ...
                  <Select value={testForm.subjectId} onValueChange={v => setTestForm(p => ({ ...p, subjectId: v }))}><SelectTrigger><SelectValue placeholder="Subject *" /></SelectTrigger><SelectContent>{subjects.map(s => <SelectItem key={s.id} value={String(s.id)}>{s.name}</SelectItem>)}</SelectContent></Select>
                  <Input placeholder="Grade *" type="number" min="1" max="10" value={testForm.grade} onChange={e => setTestForm(p => ({ ...p, grade: e.target.value }))} />
                  <Select value={testForm.courseId} onValueChange={v => setTestForm(p => ({ ...p, courseId: v }))}><SelectTrigger><SelectValue placeholder="Course (optional)" /></SelectTrigger><SelectContent><SelectItem value="">No course</SelectItem>{courses.map(c => <SelectItem key={c.id} value={String(c.id)}>{c.title}</SelectItem>)}</SelectContent></Select>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h4 className="font-semibold text-sm" style={{ color: NAVY }}>Questions ({questions.length})</h4>
                  ...
                  {questions.map((q, qi) => (
                    <div key={qi} className="rounded-xl border border-gray-200 p-4 space-y-2 bg-gray-50">
                      <div className="flex items-start gap-2">
                        ...
                        <Input placeholder="Question text *" value={q.text} onChange={e => updateQuestion(qi, "text", e.target.value)} className="flex-1 bg-white" />
                        {questions.length > 1 && <button onClick={() => removeQuestion(qi)} className="mt-1 text-red-400 hover:text-red-600"><X className="w-4 h-4" /></button>}
                      </div>
                      <div className="grid grid-cols-2 gap-2 ml-7">
                        {q.options.map((opt, oi) => (
                          <div key={oi} className="flex items-center gap-1.5">
                            <input type="radio" name={`q${qi}`} checked={q.correctOption === oi} onChange={() => updateQuestion(qi, "correctOption", oi)} className="w-3.5 h-3.5 accent-orange-500 flex-shrink-0" />
                            <Input placeholder={`Option ${String.fromCharCode(65 + oi)} *`} value={opt} onChange={e => updateOption(qi, oi, e.target.value)} className="bg-white text-xs" />
                          </div>
                        ))}
                      </div>
                ...
                <div className="flex gap-2">
                  <Button size="sm" onClick={createTest} disabled={busy || !testForm.title || !testForm.subjectId || !testForm.grade || !testForm.scheduledAt} className="text-white" style={{ background: ORANGE }}>Create Test</Button>
            ...
            <div className="space-y-3">
              {tests.map(t => (
                <div key={t.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-gray-400 mt-0.5">{t.subjectName} · Grade {t.grade} · {new Date(t.scheduledAt).toLocaleDateString("en-IN")} · {t.duration} min · {t.totalQuestions} Qs</div>
                  </div>
                  <div className="flex items-center gap-2">
              ...
              {tests.length === 0 && !showTestForm && <div className="py-12 text-center"><ClipboardList className="w-8 h-8 text-gray-300 mx-auto mb-2" /><p className="text-gray-400 text-sm">No tests created yet</p></div>}
            </div>
          </div>
        )}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {students.map((s: any, i: number) => (
                    <tr key={i} className="border-b border-gray-50 hover:bg-gray-50">
                      <td className="px-4 py-3"><div className="flex items-center gap-2">
                        ...
                        <div><div className="font-medium text-sm" style={{ color: NAVY }}>{s.studentName}</div><div className="text-xs text-gray-400">{s.email ?? s.phone ?? "—"}</div></div>
                      </div></td>
                  ...
                  {students.length === 0 && <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400 text-sm">No students enrolled yet</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        {tab === "submissions" && (
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              {submissions.length === 0 && <div className="py-12 text-center"><CheckCircle className="w-8 h-8 text-gray-300 mx-auto mb-2" /><p className="text-gray-400 text-sm">No submissions yet</p></div>}
            </div>
          </div>
        )}
        ...
        {tab === "attendance" && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-500">Select a live class from the Live Classes tab and click "Attend" to mark attendance, or pick a class below.</p>
            ...
            {attendanceClassId !== null ? (
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-orange-200 space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="font-bold text-sm" style={{ color: NAVY }}>
                    Attendance for: {liveClasses.find(c => c.id === attendanceClassId)?.title ?? `Class #${attendanceClassId}`}
                  </h4>
              ...
              <div className="space-y-2">
                ...
                {liveClasses.length === 0 && <div className="py-8 text-center"><ClipboardList className="w-8 h-8 text-gray-300 mx-auto mb-2" /><p className="text-gray-400 text-sm">No live classes yet</p></div>}
              </div>
            )}
          </div>
        )}
