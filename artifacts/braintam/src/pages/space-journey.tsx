...
export default function SpaceJourneyPage() {
  ...
  const pct = current.maxXP === Infinity
    ? 100
  ...
  return (
    <AppLayout>
      ...
      <div className="relative overflow-hidden"
        style={{ background: "linear-gradient(160deg, #050e2a 0%, #0B2B6B 60%, #0f3580 100%)" }}>
        <StarField />
        <div className="relative z-10 px-5 pt-6 pb-8 max-w-3xl mx-auto">
          <Link href="/dashboard">
            <button className="flex items-center gap-1.5 text-white/60 hover:text-white text-sm font-medium mb-5 transition-colors">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="text-3xl md:text-4xl font-black text-white">Space Journey</h1>
            ...
            <div className="flex flex-col gap-2 items-end flex-shrink-0">
              <div className="px-4 py-2 rounded-2xl text-center"
                ...
                <div className="text-white/50 text-[10px] font-bold uppercase tracking-wide">Total XP</div>
                <div className="text-xl font-black" style={{ color: "#FBBF24" }}>{xp.toLocaleString()}</div>
              </div>
              {rank && (
                <div className="px-3 py-1 rounded-full text-xs font-bold"
                  style={{ background: "rgba(255,107,26,0.2)", color: "#FDBA74", border: "1px solid rgba(255,107,26,0.3)" }}>
                  Rank #{rank}
                </div>
              )}
            </div>
          ...
          {/* Current level card */}
          <div className="mt-6 p-4 rounded-2xl" style={{ background: "rgba(255,255,255,0.07)", border: `1px solid ${current.glow}` }}>
            ...
            <div className="space-y-1.5">
              ...
              {xpNeeded > 0 && (
                <p className="text-white/40 text-xs text-center">
                  <span className="text-orange-400 font-bold">{xpNeeded} XP</span> needed to reach {LEVELS[currentIdx + 1]?.name}
                </p>
              )}
            </div>
          </div>
      ...
      {/* ── Journey Map ── */}
      <div className="min-h-screen" style={{ background: "linear-gradient(180deg, #050e2a 0%, #0a1628 100%)" }}>
        <StarField />
        <div className="relative z-10 max-w-2xl mx-auto px-4 py-8">
          ...
          <div className="relative">
            ...
            <div className="absolute left-1/2 top-0 bottom-0 w-px -translate-x-1/2 hidden md:block"
              style={{ background: "repeating-linear-gradient(to bottom, rgba(255,255,255,0.15) 0px, rgba(255,255,255,0.15) 6px, transparent 6px, transparent 14px)" }} />
                  ...
                  >
                    <div className={`relative rounded-3xl p-4 border transition-all ${
                      ...
                      style={{
                        ...
                        borderColor: isCurrent ? level.glow : undefined,
                      ...
                      <div className="absolute -top-3 left-6 px-2 py-0.5 rounded-full text-[10px] font-black"
                        style={{
                          background: isCurrent ? level.color : isCompleted ? "rgba(255,255,255,0.15)" : "rgba(255,255,255,0.05)",
                          color: isCurrent ? "white" : isCompleted ? "white" : "rgba(255,255,255,0.3)",
                        }}>
                        Level {i + 1}
                      </div>
                      ...
                      <div className="flex items-center gap-4 mt-1">
                        ...
                        <div className="relative flex-shrink-0">
                          ...
                          {isCompleted && (
                            <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-green-500 flex items-center justify-center text-white text-[10px] font-black">✓</div>
                          )}
                          {isCurrent && (
                            <motion.div
                        ...
                        <div className="flex-1 min-w-0">
                          <div className={`font-black text-base ${isLocked ? "text-white/25" : "text-white"}`}>
                            {level.name}
                          </div>
                        ...
                        <div className="flex-shrink-0 text-right">
                          <div className={`text-xs font-bold ${isLocked ? "text-white/20" : "text-yellow-400"}`}>
                            {level.reward}
                          </div>
                        ...
                        {selected === i && (
                          ...
                          >
                            <div className="mt-4 pt-4 border-t border-white/10 space-y-3">
                              <p className={`text-sm leading-relaxed ${isLocked ? "text-white/30" : "text-white/60"}`}>
                                {level.desc}
                              </p>
                              <div>
                                <p className="text-white/30 text-[10px] font-bold uppercase tracking-wider mb-2">
                                  {isCompleted ? "✅ Completed Missions" : isCurrent ? "🎯 Active Missions" : "🔒 Locked Missions"}
                                </p>
                                <div className="space-y-1.5">
                                  {level.missions.map((m, mi) => (
                                    <div key={mi} className="flex items-center gap-2">
                                      <div className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-black flex-shrink-0 ${
                                        ...
                                        : isCurrent && mi === 0 ? "bg-orange-500 text-white"
                              ...
                              <div className="flex items-center gap-2 p-3 rounded-2xl" style={{ background: "rgba(255,255,255,0.05)" }}>
                                <span className="text-lg">{isLocked ? "🔒" : isCompleted ? "✅" : level.planet}</span>
                                <div>
                                  <div className="text-white/40 text-[10px]">Unlock Reward</div>
                                  <div className="text-sm font-bold" style={{ color: isLocked ? "rgba(255,255,255,0.2)" : "#FBBF24" }}>
                                    {level.reward} · {level.rewardXP}
                                  </div>
                                </div>
                              </div>
                            </div>
          ...
          {/* How to earn XP */}
          <motion.div className="mt-10 rounded-3xl p-5"
