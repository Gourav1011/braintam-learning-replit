    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md p-0 overflow-hidden gap-0">
        <div className="p-4 border-b">
          ...
          <DialogDescription className="mt-1 text-xs text-gray-500">Only group members can see this conversation and its messages.</DialogDescription>
        </div>
        <div className="p-3 border-b bg-gray-50/60">
          ...
          {query && isLoading && <p className="mt-2 text-xs text-gray-500">Searching employees…</p>}
          {query && isError && <p className="mt-2 text-xs text-red-600">Employees could not be loaded. <button type="button" className="font-bold underline" onClick={() => { void refetch(); }}>Try again</button></p>}
          {query && !isLoading && !isError && candidates.slice(0, 5).map((employee: any) => (
            <div key={employee.id} className="mt-2 flex items-center gap-2 rounded-lg bg-white p-2 border">
              ...
              <Button size="sm" className="h-7 bg-blue-600 text-xs hover:bg-blue-700" disabled={addMember.isPending} onClick={() => addMember.mutate({ conversationId: String(conversation.id), userId: String(employee.id) }, { onError: () => toast.error("The group member could not be added.") })}>
          ...
          {members.map((member: any) => (
            <div key={member.id} className="flex items-center gap-3 rounded-lg p-2 hover:bg-gray-50">
              ...
              {member.id !== conversation.createdById && (
                <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-400 hover:text-red-600" title={`Remove ${member.name}`} disabled={removeMember.isPending} onClick={() => removeMember.mutate({ conversationId: String(conversation.id), userId: String(member.id) }, { onError: () => toast.error("The group member could not be removed.") })}>
