...
router.get("/admin/courses", adminOnly, async (req, res) => {
  const courses = await db.select({
    ...
    enrolledCount: sql<number>`(SELECT COUNT(*)::int FROM mastery_students WHERE assigned_course_id = "courses"."id")`,
    subjectsCount: sql<number>`(SELECT COUNT(*)::int FROM course_subjects WHERE course_id = "courses"."id")`,
    topicsCount: sql<number>`(SELECT COUNT(*)::int FROM topics JOIN chapters ON topics.chapter_id = chapters.id WHERE chapters.course_id = "courses"."id")`,
    teachersCount: sql<number>`(SELECT COUNT(*)::int FROM teacher_courses WHERE course_id = "courses"."id")`,
  })
    .from(coursesTable)
...
router.get("/admin/dashboard", adminOnly, async (_req, res) => {
  ...
  // Calendar week: Monday 00:00 -> next Monday 00:00.
  ...
  // mastery_students is the programme-membership source of truth.
  // student_id links Mastery records to the platform user/activity tables.
  ...
  const [
    ...
    db.select({
      count: sql<number>`count(distinct ${enrollmentsTable.id})`,
    })
      .from(enrollmentsTable)
      .innerJoin(
        coursesTable,
    ...
    db.select({
      count: sql<number>`count(distinct ${homeworkSubmissionsTable.id})`,
    })
      .from(homeworkSubmissionsTable)
      .innerJoin(
        masteryStudentsTable,
    ...
    db.select({
      count: sql<number>`count(distinct ${testSubmissionsTable.id})`,
    })
    ...
    db.select({
      ...
      .where(and(
        activeMastery,
        gte(usersTable.lastLoginDate, todayStart),
        lt(usersTable.lastLoginDate, tomorrowStart),
      )),

    // Distinct Mastery students with positive XP activity today.
    db.select({
      count: sql<number>`count(distinct ${pointsLedgerTable.userId})`,
    })
      ...
      .where(and(
        ...
        gte(pointsLedgerTable.createdAt, todayStart),
        lt(pointsLedgerTable.createdAt, tomorrowStart),
      )),
