...
**How to apply:** when debugging any staff/user/login/data issue, always query `NEON_DATABASE_URL` first, not `DATABASE_URL`. If replit.md's "Staff accounts" table hasn't been verified against Neon recently, treat it as unverified and re-check live before trusting documented credentials.
